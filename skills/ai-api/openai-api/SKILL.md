---
name: openai-api
description: OpenAI API integration for building AI-powered applications. Use when working with OpenAI's Chat Completions API, Python SDK (openai), TypeScript SDK (openai), tool use/function calling, vision/image inputs, streaming responses, DALL-E image generation, Whisper audio transcription, text-to-speech, embeddings, Assistants API, fine-tuning, or any OpenAI API integration task. Triggers on mentions of OpenAI, GPT-4, GPT-4o, GPT-5, o1, o3, o4, DALL-E, Whisper, Sora, or OpenAI SDK usage. DO NOT use for Google Gemini API - use gemini-api skill instead.
---
...
| `max_completion_tokens` | Maximum response length (newer models) |
...
**IMPORTANT:** GPT-5 series and reasoning models have different parameter support from GPT-4 series.
...
|-----------|---------|
| `max_tokens` | **NOT supported**. Use `max_completion_tokens` instead |
| `temperature` | Fixed at 1. Do NOT pass this parameter |
| `top_p` | Fixed at 1. Do NOT pass this parameter |
| `system` role | **NOT supported** for o1. Use `developer` role or `user` role instead |
| `stream` | Supported (o3, o4-mini). Not supported (o1) |

### GPT-4o / GPT-4o-mini / GPT-4.1 series

All standard parameters supported (`temperature`, `max_tokens`, `top_p`, `stop`, `n`, etc.).
