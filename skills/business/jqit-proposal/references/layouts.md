...
<!-- IMAGE SLOT: ヒーロー画像（抽象ビジュアル） / 右半分 or 背景 / 必須 -->
...
<body>
<!-- 白背景 -->
<div style="position: absolute; width: 720pt; height: 405pt; background: #FFFFFF;"></div>
<!-- JQIT赤サイドバー -->
...
<div style="position: absolute; left: 40pt; top: 100pt; width: 640pt;">
  <h1 style="color: #1C2833; font-size: 30pt; font-weight: bold; line-height: 1.3; margin: 0;">{{タイトル}}</h1>
</div>
...
<div style="position: absolute; left: 40pt; top: 215pt; width: 500pt;">
  <p style="color: #5D6D7E; font-size: 14pt; margin: 0;">{{サブタイトル}}</p>
</div>

<!-- ボトム情報 -->
<div style="position: absolute; left: 40pt; bottom: 40pt;">
  <p style="color: #AAB7B8; font-size: 10pt; margin: 0 0 4pt 0;">{{日付}}</p>
  <p style="color: #AAB7B8; font-size: 10pt; margin: 0;">{{会社名}}</p>
</div>
<img src="logo.png" style="position: absolute; right: 30pt; bottom: 20pt; height: 22pt;">
</body></html>
```
...
- カード枚数: 最大5枚（6枚以上は溢れる）
- ラベル: 最大15文字
- 内容テキスト: 最大80文字/カード（2行以内）
- 内容が長い場合は4枚以下に減らしてカードあたりの高さを確保
...
- カード枚数: 固定4枚
- 見出し: 最大20文字/カード
- 説明文: 最大2行（各行30文字以内）/カード
- 説明が3行以上必要な場合はcard-listレイアウトを検討

<!-- IMAGE SLOT: データビジュアライゼーション / 全面画像で置換 / 任意 -->
...
- カード枚数: 固定3枚
- カードタイトル: 最大12文字/カード
- 箇条書き: 最大5項目/カード（各項目20文字以内）
- サブヘッダー: 最大30文字

<!-- IMAGE SLOT: 機能インフォグラフィック / ヘッダー下・全幅 / 推奨 -->
...
**コンテンツ上限**:
- 入力/出力ボックス: 各1行（30文字以内）
- システムボックス内の機能タグ: 最大4個（各タグ10文字以内）
- ボトムカード: 最大3枚（タイトル12文字+説明20文字以内）
- 全体の縦方向要素数: 入力+矢印+システム+矢印+出力+ボトムカード = 最大構成。ボトムカードを使う場合は補足テキストを省略

<!-- IMAGE SLOT: プロセスフロー図 / 全面画像で置換 / 推奨 -->
...
**コンテンツ上限**:
- チェックリスト項目: 最大5項目（各項目40文字以内）
- ステップバリエーション: 最大4ステップ（タイトル15文字+説明30文字以内）
- タイトル+サブテキスト+セクション見出し+チェック項目+ボトムテキスト全体で縦405pt内に収める

<!-- IMAGE SLOT: 装飾的背景 / 右側アクセント / 任意 -->
...
**コンテンツ上限**:
- 説明ボックス: タイトル15文字+テキスト40文字以内（省略可）
- 左右各カラム: 最大4項目（タイトル15文字+説明25文字以内）
- 項目数が5以上の場合はcard-listレイアウトに変更を検討

<!-- IMAGE SLOT: 比較チャート / 全面画像で置換 / 任意 -->
...
- タスクラベル: 最大10文字
- タスク内容: 最大15文字
- 4フェーズ以上の場合は2スライドに分割するか、Gemini画像で全体ガントチャートを生成

<!-- IMAGE SLOT: ガントチャート/ロードマップ / 全面画像で置換 / 推奨 -->
...
| スライドの目的 | 推奨レイアウト | 画像推奨 |
|---------------|---------------|----------|
| 表紙 | title | 必須（ヒーロー画像） |
| サマリー / 要点 | card-list | 不要 |
| 4つの課題 / メリット | card-grid-2x2 | 任意（データビジュアライゼーション） |
| 3つの機能 / ポイント | card-3col | 推奨（機能インフォグラフィック） |
| システム構成 / プロセス | flow-diagram | 推奨（プロセスフロー図） |
| デモ紹介 / CTA | dark-checklist | 任意（装飾的背景） |
| スコープ内外 / 比較 | two-column | 任意（比較チャート） |
| スケジュール | timeline | 推奨（ガントチャート） |
| 次のステップ | dark-checklist（ステップバリエーション） | 任意（装飾的背景） |
| 会社紹介 | card-list | 不要 |
| セキュリティ | card-list | 不要 |
| 費用 | card-list + テーブル（placeholder） | 任意（ドーナツチャート） |
